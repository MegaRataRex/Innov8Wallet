...
type TransferAmountScreenRouteProp = RouteProp<RootStackParams, 'TransferAmountScreen'>

type TransferAmountScreenNavigationProp = StackNavigationProp<RootStackParams, 'TransferAmountScreen'>

export const TransferAmountScreen = () => {
  const navigation = useNavigation<TransferAmountScreenNavigationProp>();
  const route = useRoute<TransferAmountScreenRouteProp>();
  const { contact } = route.params;

  const [amount, setAmount] = useState('');
  const [availableBalance] = useState(19523.00); // This would come from your app state or API
  ...
  const handleContinue = () => {
    ...
    navigation.navigate('TransferConfirmationScreen', { contact, amount: parseFloat(amount) });
    console.log(`Transferring $${amount} to ${contact.name}`);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.subtitle}>¿Que cantidad desea transferir?</Text>
      ...
      <View style={styles.amountContainer}>
        ...
        >
          <Image
            source={require('../../assets/icons/right-arrow.png')} // Replace with your arrow icon
